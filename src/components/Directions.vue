...
export default {
  ...
  methods: {
    getData(page, rows) {
      this.$axios
        .post("/management/sysaskroad/getPageListByParam", {
          pageSize: rows,
          startPage: page,
    ...
    expandChange(row){
      console.log(row)
      this.$axios.post(`/management/sysaskroad/getByGroup?groupStr=${row.groupStr}`).then(res=>{
        if(res.status==200){
          this.formLabelAlign = res.data.data.data
          this.$set(this.formLabelAlign,'intention',res.data.data.intention)
        }
      })
    },
    open() {
    ...
    removeDomain(item) {
      ...
      if(item.id &&index2!=-1){
        ...
        this.$axios
          .post(
            `/management/sysquestionans/deleteById?questionId=${item.id}`
          )
          .then(res => {
            if (res.status == 200) {
    ...
    saveSet() {
      ...
      console.log(this.formLabelAdd.synonymBos2[this.idx])
    ...
    removeDomain2(item) {
      ...
      if(item.id &&index2!=-1){
        this.$axios
          .post(
            `/management/sysaskroad/deleteById?questionId=${item.id}`
          )
          .then(res => {
            if (res.status == 200) {
    ...
    handleAdd(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$axios
            .post("/management/sysaskroad/saveOrUpdate", params)
            .then(res => {
              if (res.status == 200) {
    ...
    handleEdit(index, row,flag) {
      ...
      this.$axios.post(`/management/sysaskroad/getByGroup?groupStr=${row.groupStr}`).then(res=>{
        ...
        if(res.status==200){
          ...
          this.$set(this.formLabelEdit,'intention',res.data.data.intention)
...
saveEdit(formName){
  ...
  this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          console.log(params.toString());
          this.$axios
            .post(`/management/sysaskroad/saveOrUpdate?id=${this.idx}`, params)
            .then(res => {
              if (res.status == 200) {
...
// 删除
    handleDelete(index, rows) {
      ...
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
        ...
        this.$axios
          .post(
            `/management/sysaskroad/deleteByGroup?groupStr=${rows.groupStr}`
          )
          .then(res => {
            if (res.status == 200) {
              this.$message.success("删除成功");
